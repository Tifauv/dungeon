...
use crate::dungeon::{Orientation, Player};
...
impl<'s> System<'s> for MovePlayerSystem {
	...
	fn run(&mut self, (mut players, mut locals, input, time): Self::SystemData) {
			...
			if let Some(movement) = input.axis_value("player_walk") {
				...
				if movement != 0.0 {
					...
					match player.orientation {
						Orientation::East  => local.set_translation_x(player_x + movement),
						Orientation::North => local.set_translation_y(player_y - movement),
						Orientation::West  => local.set_translation_x(player_x - movement),
						Orientation::South => local.set_translation_y(player_y + movement)
					};
